...
labels = myfile[1:, 1] # alive or dead
num_label = 2
...
estimators = [('logstic_regression', LogisticRegression(solver='lbfgs', n_jobs=2, multi_class='auto',max_iter=1000)),
              ('SVC_poly3', svm.SVC(kernel='poly', degree=3, gamma='auto', max_iter=10000)),
              ('SVC_linear', svm.SVC(kernel='linear', gamma='auto', max_iter=10000)),
...
pca_sets = [('PCA, var_ratio_acc = 0.5', PCA(n_components=0.5)),
            ('PCA, var_ratio_acc = 0.9', PCA(n_components=32)),
            ('PCA, var_ratio_acc = 0.98', PCA(n_components=0.98)),]
...
for name_pca, method_pca in pca_sets:
    ...
    for i in range(1):
        print("******************  %d   ******************" % i)
        data_train, data_test, label_train, label_test = train_test_split(data_pca, labels, test_size=0.25, random_state=None, shuffle=True)
